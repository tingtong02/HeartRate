...
def preprocess_ppg_for_beats(samples: np.ndarray, fs: float, config: dict[str, Any] | None = None) -> np.ndarray:
    cfg = config or {}
...
def refine_beats_in_window(ppg_window: np.ndarray, peak_indices: np.ndarray, fs: float, config: dict[str, Any] | None = None) -> np.ndarray:
    cfg = config or {}
...
def _prune_close_beats(signal: np.ndarray, peak_indices: np.ndarray, min_distance: int) -> np.ndarray:
...
def detect_beats_in_window(ppg_window: np.ndarray, fs: float, config: dict[str, Any] | None = None) -> np.ndarray:
    cfg = config or {}
    processed = preprocess_ppg_for_beats(ppg_window, fs=fs, config=cfg)
    if processed.size < 8:
        return np.array([], dtype=int)
    ...
    refined = refine_beats_in_window(processed, peaks, fs=fs, config=cfg)
    prune_distance = max(int(round(min_distance * float(cfg.get("refractory_scale", 1.5)))), min_distance)
    return _prune_close_beats(processed, refined, prune_distance)
...
def clean_ibi_series(ibi_s: np.ndarray, config: dict[str, Any] | None = None) -> dict[str, np.ndarray | bool]:
    cfg = config or {}
